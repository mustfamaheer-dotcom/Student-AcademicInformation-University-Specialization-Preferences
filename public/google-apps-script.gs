...
// Helper to normalize digits (convert Arabic digits to ASCII digits and strip spaces/quotes)
function normalizeDigits(valStr) {
  if (!valStr) return "";
  var str = valStr.toString().trim();
...
function doPost(e) {
  ...
  try {
    ...
    var incomingNationalId = normalizeDigits(data.nationalId || "");
    var incomingPhone = normalizeDigits(data.phone || "");

    // Duplicate Check for National ID (Column 3) & Mobile Phone (Column 4)
    var lastRow = sheet.getLastRow();
    if (lastRow > 1) {
      var rangeValues = sheet.getRange(2, 3, lastRow - 1, 2).getValues();
      for (var r = 0; r < rangeValues.length; r++) {
        var existingId = normalizeDigits(rangeValues[r][0]);
        var existingPhone = normalizeDigits(rangeValues[r][1]);

        if (incomingNationalId && existingId === incomingNationalId) {
          return ContentService
            .createTextOutput(JSON.stringify({
              "result": "error",
              "code": "DUPLICATE_NATIONAL_ID",
              "message": "عذراً، غير مسموح بالاستخدام أو التسجيل أكثر من مرة بنفس الرقم القومي (" + incomingNationalId + ")."
            }))
        ...
        if (incomingPhone && existingPhone === incomingPhone) {
          return ContentService
            .createTextOutput(JSON.stringify({
              ...
              "message": "عذراً، غير مسموح بالاستخدام أو التسجيل أكثر من مرة بنفس رقم التليفون (" + incomingPhone + ")."
            }))
            .setMimeType(ContentService.MimeType.JSON);
        }
      }
    ...
    var row = [
      timestamp,
      data.fullName || "",
      "'" + incomingNationalId, // Prefix with apostrophe to preserve formatting in Google Sheets
      "'" + incomingPhone,
      data.seatNumber || "",
      data.specialization || "",
...
function doGet(e) {
  if (e && e.parameter) {
    ...
    var targetId = normalizeDigits(e.parameter.id || e.parameter.nationalId || "");
    var targetPhone = normalizeDigits(e.parameter.phone || "");

    if (action === "checkUser" || action === "checkNationalId" || action === "checkPhone") {
      var idExists = false;
      var phoneExists = false;
      try {
        var sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
        var lastRow = sheet.getLastRow();
        if (lastRow > 1 && (targetId || targetPhone)) {
          var rangeValues = sheet.getRange(2, 3, lastRow - 1, 2).getValues();
          for (var r = 0; r < rangeValues.length; r++) {
            var existingId = normalizeDigits(rangeValues[r][0]);
            var existingPhone = normalizeDigits(rangeValues[r][1]);
            if (targetId && existingId === targetId) idExists = true;
            if (targetPhone && existingPhone === targetPhone) phoneExists = true;
            if (idExists && phoneExists) break;
          }
        }
      } catch (err) {}
